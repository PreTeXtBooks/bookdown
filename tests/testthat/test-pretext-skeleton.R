...
test_that("PreTeXt components chapter mirrors the Rmd structure", {
  ...
  expect_match(components, "```{r no-caption, fig.width=6, fig.asp=.7, out.width='70%', ref.label='pressure-plot'}", fixed = TRUE)
  expect_match(components, "There are a few advantages of using <c>include_graphics()</c>:", fixed = TRUE)
  expect_match(components, "```{r longtable, tidy=FALSE}", fixed = TRUE)
  ...
  expect_match(components, "bibliography: [\"one.bib\", \"another.bib\", \"yet-another.bib\"]", fixed = TRUE)
  ...
  expect_match(components, "knitr::include_url()", fixed = TRUE)
